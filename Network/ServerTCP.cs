        ...
        public void ServerInit()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        Console.WriteLine("EMPTY COMMAND IP: {0}", s.RemoteEndPoint);
                        continue;
                    }
                    ...
                    if (user == null)
                    {
                        Console.WriteLine("COMMAND FROM NON USER IP: {0}", s.RemoteEndPoint);
                        continue;
                    }
                    ...
                    switch (values[0])
                    {
                        ...
                        case "Queue:":
                            ...
                            YouTubeInfo toQueue;            
                            using (MemoryStream msq = new MemoryStream(bArray, commandMessagePos, bArray.Length - commandMessagePos))
                            using (BsonReader reader = new BsonReader(msq))
                            {
                        ...
                        case "GetQueue":
                            ...
                            {
                                MemoryStream msgq = new MemoryStream();
                                using (BsonWriter writer = new BsonWriter(msgq))
                                {
                                    JsonSerializer serializer = new JsonSerializer();
                                    //serializer.Serialize()
                                    serializer.Serialize(writer, ytInfo, typeof(QueuePieceToSend[]));
                                    Console.WriteLine("TCP/ Sending queue to: {0} : {1}", s.RemoteEndPoint, Convert.ToBase64String(msgq.ToArray()));
                                    s.Send(msgq.ToArray());
                                }
                            }
                            break;
                        ...
                        case "GetUsers":
                            ...
                            using (BsonWriter writer = new BsonWriter(ms))
                            {
                                ...
                                Console.WriteLine("TCP/ Sending users to: {0} : {1}", s.RemoteEndPoint, Convert.ToBase64String(ms.ToArray()));
                                s.Send(ms.ToArray());
                            }
                        ...
                        case "ChangeName":
                            ...
                            {
                                Console.WriteLine("Changing User {0} to {1}", user.Name, values[1]);
                                user.Name = values[1];
                                Application.Current.Dispatcher.Invoke(new Action(() => { QueueManager.userChangeName(user); }));
                            ...
                            {
                                Console.WriteLine("TCP/ Other user with same name; Sending 0");
